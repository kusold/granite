// Pure date math for the calendar panel and the bar clock's format ring,
// ported from Omarchy Quattro's clock plugin
// (shell/plugins/panels/clock/Model.js, MIT). Locale- and Qt-free so the
// QML owns month/weekday naming through Qt.locale(); everything here could
...
// ---- Bar label formats. Right-clicking the clock walks these in order;
//      the walk is session-only here (no shell.json settings store to
//      persist a cycled format into).
//
...
// Whether a format prints seconds, so the clock ticks once a second only
// for the formats that show them. Quoted literals go first: the s in a
// 'Sat' is text rather than a token, and an opening quote with no closing
// one runs to the end of the format the way Qt reads it.
...
// starts the walk at the top; the fixed order keeps the walk from bouncing
// between two entries.
function nextClockFormat(current) {
  var text = String(current === undefined || current === null ? "" : current)
  var index = CLOCK_FORMATS.indexOf(text)
...
// the value is missing or nonsense.
...
// between. Any other start lands on Monday the first time it is toggled.
