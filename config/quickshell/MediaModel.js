// Pure media logic for the M8 plugin, ported from Omarchy Quattro's
// shell/plugins/services/media/MediaModel.js
// (https://github.com/basecamp/omarchy, MIT) at granite's scale: same
// player classification (proxy players, metadata tiers, playback-stream
// matching), track signatures, and OSD message shape, minus their
// source-cycling predicates (granite has no media-source keybinds yet).
// The service that owns the reactive half is Media.qml; this file stays
// side-effect free so the ladder is testable by reading it.

// playerctld (and anything fronting other players the same way) is a
// proxy: ranking treats it as a fallback everywhere, so it only wins when
// no real player can.
function isProxyPlayer(player) {
  var dbusName = String(player && player.dbusName || "").toLowerCase()
  var desktopEntry = String(player && player.desktopEntry || "").toLowerCase()
  return dbusName.indexOf("playerctld") !== -1 || desktopEntry === "playerctld"
}

function hasMetadata(player) {
  return !!(player && (player.trackTitle || player.trackArtist || player.identity || player.desktopEntry))
}

function hasTrackMetadata(player) {
  return !!(player && (player.trackTitle || player.trackArtist || player.trackAlbum || player.trackArtUrl))
}

function playerCanControl(player) {
  return !!(player && (player.canTogglePlaying || player.canPlay || player.canPause || player.canGoNext || player.canGoPrevious))
}

function canHandleAction(player, action) {
  if (!player) return false
  if (action === "next") return !!player.canGoNext
  if (action === "previous") return !!player.canGoPrevious
  if (action === "play") return !!player.canPlay || (!!player.canTogglePlaying && !player.isPlaying)
  if (action === "pause") return !!player.canPause || (!!player.canTogglePlaying && player.isPlaying)
  if (action === "playPause")
    return !!player.canTogglePlaying || !!player.canPlay || !!player.canPause
  return false
}

// ----- pipewire playback streams -------------------------------------------
//
// Identify playback streams without reading node.properties here:
// PwNode.properties is invalid until the node is bound, and reading it
// while capture streams are appearing can destabilize Quickshell's
// Pipewire service. Quickshell versions differ in how `type` is exposed
// (media.class, enum name, or numeric enum), but playback streams
// consistently accept audio input from clients and publish `isSink: true`;
// capture streams publish as stream sources.

function nodeProps(node) {
  return node && node.ready && node.properties ? node.properties : {}
}

function isPlaybackStream(node) {
  if (!node || !node.isStream) return false
  if (node.isSink === true) return true

  var mediaClass = String(node.type || "")
  return mediaClass.indexOf("Stream/Output/Audio") !== -1
    || mediaClass.indexOf("AudioOutStream") !== -1
    || mediaClass.indexOf("Output") !== -1
}

function rawStreamLabel(node) {
  if (!node) return ""
  var p = nodeProps(node)
  return p["application.name"]
    || node.description
    || p["media.name"]
    || p["node.name"]
    || node.name
}

// "Firefox" vs "pipewire alsa [Firefox]" vs "alsa playback [mpv]" — one
// alphanumeric key so either spelling matches the other.
function streamLabelKey(label) {
  var key = String(label || "").toLowerCase()
  key = key.replace(/^pipewire alsa \[/, "")
  key = key.replace(/\]$/, "")
  key = key.replace(/^alsa playback \[/, "")
  key = key.replace(/[^a-z0-9]+/g, "")
  return key
}

// The player's app identity in stream-label spelling: the desktop entry
// first (Firefox, Spotify), else the MPRIS identity, else the dbus name
// minus its org.mpris.MediaPlayer2. prefix and .instanceN suffix.
function playerAppLabel(player) {
  if (!player) return ""
  var dbus = String(player.dbusName || "")
  dbus = dbus.replace(/^org\.mpris\.MediaPlayer2\./, "")
  dbus = dbus.replace(/\.instance[0-9]+$/, "")
  return player.desktopEntry || player.identity || dbus
}

function playerHasPlaybackStream(player, playbackStreams) {
  var key = streamLabelKey(playerAppLabel(player))
  if (!key) return false

  var streams = Array.isArray(playbackStreams) ? playbackStreams : []
  for (var i = 0; i < streams.length; i++) {
    var streamKey = streamLabelKey(rawStreamLabel(streams[i]))
    if (!streamKey) continue
    if (streamKey === key
        || streamKey.indexOf(key) !== -1
        || key.indexOf(streamKey) !== -1)
      return true
  }

  return false
}

// ----- identity, ordering, messages ----------------------------------------

function playerKey(player) {
  if (!player) return ""
  return String(player.dbusName || player.desktopEntry || player.identity || "")
}

function trackSignature(player) {
  if (!player) return ""
  return [
    player.trackTitle || "",
    player.trackArtist || "",
    player.trackAlbum || "",
    player.trackArtUrl || ""
  ].join("\u001f")
}

function trackChanged(previousSignature, player) {
  return trackSignature(player) !== String(previousSignature || "")
}

function labelFor(player) {
  if (!player) return ""
  return player.trackTitle || player.identity || player.desktopEntry || ""
}

// The OSD's "Title - Artist" shape: the track when there is one, else the
// player identity, else the action label the caller passed as fallback.
function osdMessage(player, fallback) {
  if (!player) return fallback
  var label = labelFor(player)
  if (label.length > 0 && player.trackArtist) return label + " - " + player.trackArtist
  return label.length > 0 ? label : fallback
}
